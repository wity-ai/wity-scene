#!/usr/bin/env bash
# deploy.sh — Build and deploy witySceneRender gateway to AWS Lambda
#
# Usage:  ./services/scene-render/gateway/deploy.sh
...
# Gateway has no layers (no FFmpeg needed)
LAYER_FLAGS=()

# Environment: downstream Lambda function names
SCENE_TO_VIDEO_FN=$(jq -r '.environment.SCENE_TO_VIDEO_FUNCTION' "$CONFIG")
SCENE_COMPOSE_FN=$(jq -r '.environment.SCENE_COMPOSE_FUNCTION'  "$CONFIG")
...
echo "──────────────────────────────────────────────────"
echo "  wity-scene-render gateway  →  AWS Lambda"
echo "  Function  : $FUNCTION_NAME"
echo "  Region    : $REGION"
echo "  Runtime   : $RUNTIME ($ARCHITECTURE)"
echo "  Memory    : ${MEMORY_SIZE} MB   Timeout: ${TIMEOUT}s"
echo "  Routes to : $SCENE_TO_VIDEO_FN, $SCENE_COMPOSE_FN"
echo "──────────────────────────────────────────────────"
echo ""
...
# Copy workspace packages as local deps
cp -r "$REPO_ROOT/packages/scene-core" "$BUILD_DIR/_scene-core"
rm -rf "$BUILD_DIR/_scene-core/node_modules" "$BUILD_DIR/_scene-core/dist"

# Copy the lambda handler
cp "$SCRIPT_DIR/handler.js" "$BUILD_DIR/handler.js"

# Self-contained package.json
LAMBDA_SDK_VERSION=$(jq -r '.dependencies["@aws-sdk/client-lambda"]' "$SCRIPT_DIR/package.json")
cat > "$BUILD_DIR/package.json" << EOF
{
  "name": "scene-render-lambda",
  "version": "1.0.0",
  "type": "module",
  "dependencies": {
    "@wity/scene-core":       "file:./_scene-core",
    "@aws-sdk/client-lambda": "$LAMBDA_SDK_VERSION"
  }
}
EOF
...
ENV_VARS="Variables={SCENE_TO_VIDEO_FUNCTION=${SCENE_TO_VIDEO_FN},SCENE_COMPOSE_FUNCTION=${SCENE_COMPOSE_FN}}"

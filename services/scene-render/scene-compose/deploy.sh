#!/usr/bin/env bash
# deploy.sh — Build and deploy @wity/scene-compose to AWS Lambda
#
# Usage:  ./services/scene-render/scene-compose/deploy.sh
...
echo "──────────────────────────────────────────────────"
echo "  wity-scene-compose  →  AWS Lambda"
...
cp -r "$REPO_ROOT/packages/scene-compose" "$BUILD_DIR/_scene-compose"
rm -rf \
  "$BUILD_DIR/_scene-core/node_modules" \
  "$BUILD_DIR/_scene-compose/node_modules" \
  "$BUILD_DIR/_scene-core/dist"
...
XMLDOM_VERSION=$(jq -r '.dependencies["@xmldom/xmldom"]' "$SCRIPT_DIR/package.json")
FFMPEG_VERSION=$(jq -r '.dependencies["fluent-ffmpeg"]' "$SCRIPT_DIR/package.json")
cat > "$BUILD_DIR/package.json" << EOF
{
  "name": "scene-compose-lambda",
  "version": "1.0.0",
  "type": "module",
  "dependencies": {
    "@wity/scene-core":    "file:./_scene-core",
    "@wity/scene-compose": "file:./_scene-compose",
    "@aws-sdk/client-s3":  "$S3_SDK_VERSION",
    "@xmldom/xmldom":      "$XMLDOM_VERSION",
    "fluent-ffmpeg":       "$FFMPEG_VERSION"
  }
}
EOF
